...
module.exports = {
    ...
    execute(message, args) {
        ...
        try {
            ...
            embed.setImage(gif);
            if (!muser) { return message.channel.send('nope') }
            else if (muser.user.id == message.author.id) embed.setTitle(`${message.author.bot.username} im gonna hug you ${muser.username}!`)
            else embed.setTitle(`${message.author.username} hugs ${muser.username}`)
            console.log('aa')
